...
import com.nmm.chart.v2.ext.allTrue
import com.nmm.chart.v2.ext.oneTrue
import com.nmm.chart.v2.ext.smartPost
...
object LoadingEventHelper {
    ...
    fun bindLoadingEvent(
        ...
        liveData: LiveData<XpxLoadingProgress>,
        ...
        val observer = Observer<XpxLoadingProgress> {
            when (it.event) {
    ...
    fun mergeMultiLiveData(
        owner: LifecycleOwner,
        @Size(min = 1) vararg liveDatas: LiveData<XpxLoadingProgress>
    ): LiveData<XpxLoadingProgress> {
        if (liveDatas.isEmpty()) {
            throw IllegalArgumentException("至少要有一个liveData")
        }
        val ld = MutableLiveData<XpxLoadingProgress>()

        liveDatas.forEach { liveData ->
            val observer = Observer<XpxLoadingProgress> { progress ->
                when (progress.event) {
                    DataLoadingEvent.LOADING -> {
                        //其他的有没有loading中
                    ...
                    DataLoadingEvent.SUCCESS, DataLoadingEvent.EMPTY -> {
                        ...
                        if (allOk) {
                            //全部都成功才会发送成功事件
                            ld.smartPost(XpxLoadingProgress(DataLoadingEvent.SUCCESS))
                        }
                    }
                    DataLoadingEvent.FAILURE -> {
                        //其他的有没有失败
